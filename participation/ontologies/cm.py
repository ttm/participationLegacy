# file for the caixa magica conceptualization
import percolation as P
import datetime

def makeFirstCaixaMagicaOntology(fpath):
    tg=P.rdf.makeBasicGraph([["po","cm"],[P.rdf.ns.po,P.rdf.ns.cm]],"Conceitualização da caixa mágica para a implementação inicial")
    tg2=P.rdf.makeBasicGraph([["po","cm"],[P.rdf.ns.po,P.rdf.ns.cm]],"Metadados da conceitualização da caixa mágica para a implementação inicial")
    aname="MagicBoxFirst"
    ind=P.rdf.IC([tg2],P.rdf.ns.po.Ontology,
            "MagicBox","Magic Box First Ontology")
    P.rdf.link([tg2],ind,"Magic Box First Ontology",
                          [
                          ...
                          [
                           ...
                           "Caixa Mágica group conceptualization at LabicBR (Nov/2015)",
                                "a conceptualization of the first system to be implemented in the Magic Box",
                           ])



    P.rdf.C([tg],P.rdf.ns.cm.Problem,
            "Problem",None,
            "a problem reported by the community",
            "Problema",
            "um problema apresentado pela comunidade",
            graph_lang="pt")

    P.rdf.C([tg],P.rdf.ns.cm.Solution,
            "Solution",None,
            "a solution for a problem reported by the community",
            "Solução",
            "uma solução para um problema apresentado pela comunidade",
            graph_lang="pt")

    P.rdf.C([tg],P.rdf.ns.cm.Participant,
            "Participant",None,
            "a participant of a participation process",
            "Participante",
            "um participante do processo de participação",
            graph_lang="pt")

    P.rdf.C([tg],P.rdf.ns.cm.Vote,
            "Vote",None,
            "a vote in a participation process",
            "Voto",
            "um voto no processo de participação",
            graph_lang="pt")

    P.rdf.C([tg],P.rdf.ns.cm.IDontHave,
            "I don't have",P.rdf.ns.cm.Problem,
            "the problem is something that the individual do not have",
            "Não tenho",
            "o problema é algo que o indivíduo não tem",
            graph_lang="pt")

    P.rdf.C([tg],P.rdf.ns.cm.EnvironmentDontHave,
            "The environment don't have",P.rdf.ns.cm.Problem,
            "the problem is something that the environment do not have",
            "Não tem",
            "o problema é algo que o ambiente não tem",
            graph_lang="pt")

    P.rdf.C([tg],P.rdf.ns.cm.DontExist,
            "Don't exist",P.rdf.ns.cm.Problem,
            "the problem is something that do not exist",
            "Não existe",
            "o problema é algo que não existe",
            graph_lang="pt")

    P.rdf.C([tg],P.rdf.ns.cm.DontLike,
            "Don't like",P.rdf.ns.cm.Problem,
            "the problem is something that the individual or the community does not like",
            "Não gosta",
            "o problema é algo que o indivíduo ou a comunidade não gosta",
            graph_lang="pt")

    P.rdf.P([tg],P.rdf.ns.cm.perform,"perform","faz","a participative action")
    P.rdf.L([tg],"Participante","faz","Voto")

    P.rdf.P([tg],P.rdf.ns.cm.reports,"reports","relata","an information given by a participant")
    P.rdf.L([tg],"Participante","relata","Problema")

    P.rdf.P([tg],P.rdf.ns.cm.solves,"solves","resolve")
    P.rdf.L([tg],"Solução","resolve","Problema")

    P.rdf.P([tg],P.rdf.ns.cm.associated,"associated","associado")
    P.rdf.L([tg],"Voto","associado","Problema")
    P.rdf.L([tg],"Voto","associado","Solução")

    P.rdf.LD([tg],"Problema","desempenho","xsd:float")
    P.rdf.LD([tg],"Solução","desempenho","xsd:float")

    aname="caixaMagicaFirst"
    P.rdf.writeAll(tg,aname+"Translate",fpath,1)
    P.rdf.writeAll(tg2,aname+"Meta",fpath,1)

def makeCaixaMagicaOntology(fpath):
